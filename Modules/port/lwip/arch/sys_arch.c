...
void *lwip_hook_ip4_route_src(const void *src, const void *dest) {
    const ip4_addr_t *sipaddr = (const ip4_addr_t *)src;
    const ip4_addr_t *dipaddr = (const ip4_addr_t *)dest;

    if (ip4_addr_isany(sipaddr)) return NULL;
