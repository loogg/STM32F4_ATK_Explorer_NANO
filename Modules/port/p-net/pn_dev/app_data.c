...
static uint8_t input1data[APP_GSDML_INTPUT1_DATA_SIZE] = {0};
   ...
   uint8_t * iops)
{
   ...
   if (submodule_id == APP_GSDML_SUBMOD_ID_INPUT1) {
      /* Prepare input1 data */
      mbtcp_copy_input_registers((uint16_t *)input1data, APP_GSDML_INTPUT1_DATA_SIZE / 2);
   ...
   uint16_t size)
{
   ...
   else if (submodule_id == APP_GSDML_SUBMOD_ID_ECHO)
   {
      ...
      if (size == APP_GSDML_OUTPUT1_DATA_SIZE) {
         mbtcp_set_output_register((uint16_t *)data, APP_GSDML_OUTPUT1_DATA_SIZE / 2);
         return 0;
      }
   }
