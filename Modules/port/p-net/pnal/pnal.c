...
static pnal_eth_mau_t calculate_mau_type (uint8_t link_state)
{
   switch (link_state)
   {
   case ETH_DEVICE_PHY_LINK:
      return PNAL_ETH_MAU_COPPER_10BaseT;
   case ETH_DEVICE_PHY_LINK | ETH_DEVICE_PHY_FULL_DUPLEX:
      return PNAL_ETH_MAU_COPPER_10BaseT;
   case ETH_DEVICE_PHY_LINK | ETH_DEVICE_PHY_100M:
      return PNAL_ETH_MAU_COPPER_100BaseTX_HALF_DUPLEX;
   case ETH_DEVICE_PHY_LINK | ETH_DEVICE_PHY_100M | ETH_DEVICE_PHY_FULL_DUPLEX:
...
int pnal_eth_get_status (const char * interface_name, pnal_eth_status_t * status)
{
   ...
   status->autonegotiation_advertised_capabilities =
      calculate_capabilities (ethif->phy_an_ar);

   status->operational_mau_type = calculate_mau_type (ethif->link_status);
   status->running = (ethif->link_status & ETH_DEVICE_PHY_LINK) ? true : false;
